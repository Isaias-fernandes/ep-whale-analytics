(()=> {
  ...
  const LOOKBACK=168; // H1 candles = 7 days
  const NEAR_PCT=8;
  ...
  function analyze(x,candles){
    const c=(candles||x.candles||[]).slice(-LOOKBACK);
    ...
    const retraced=high-current;
    const dropFromHigh=high?retraced/high*100:NaN;
    ...
    return {asset:x.key,sym:x.sym,low,high,current,leg,retraced,dropFromHigh,distLow,giveback,givebackStatus,tier,status,lowTs:c[lowI].t,highTs:c[highI].t,ts:Date.now()};
  }
  ...
  async function scan(){
    ...
    const results=await Promise.all(items.map(async x=>{try{return analyze(x,await h1Candles(x.sym))}catch(e){console.warn('EP Retest H1',x.sym,e);return analyze(x)}}));
    results.forEach(r=>{if(r)rows.push(r)});
    rows.sort((a,b)=>b.giveback-a.giveback||b.dropFromHigh-a.dropFromHigh);
    const now=Date.now();
    db.events=db.events||[];
    for(const r of rows.filter(x=>x.status.startsWith('RETESTE'))){
      ...
      for(const h of [1,4,24]){if(!e['r'+h]&&now-e.startTs>=h*3600000)e['r'+h]=outcome(e,h)}
      if(now-e.startTs>=86400000)e.closed=true;
    }
    db.events=db.events.slice(-500);db.rows=rows;db.updatedAt=now;render(rows,map.size);save();
  ...
  function render(rows,total=window.CryptoApp?.getData?.()?.size||0){
    ...
    el.innerHTML=diag+(show.length?`<div style="overflow:auto"><table class="price-track"><thead><tr><th>Ativo</th><th>Pernada</th><th>Classe devolução</th><th>Mínima</th><th>Máxima</th><th>Atual</th><th>Valor recuado</th><th>Queda máx.</th><th>Devolução pernada</th><th>Dist. mínima</th></tr></thead><tbody>${show.map(r=>`<tr><td><b>${r.asset}</b></td><td><b>+${f(r.leg,1)}% (≥${r.tier}%)</b></td><td><b>${r.givebackStatus}</b></td><td>${f(r.low,r.low<10?6:2)}</td><td>${f(r.high,r.high<10?6:2)}</td><td>${f(r.current,r.current<10?6:2)}</td><td><b>${f(r.retraced,r.retraced<10?6:2)}</b></td><td>${f(r.dropFromHigh,1)}%</td><td><b>${f(r.giveback,1)}%</b></td><td>${f(r.distLow,1)}%</td></tr>`).join('')}</tbody></table></div>`:'Nenhum ativo atingiu agora os critérios de devolução/reteste para exibição.');
  }
